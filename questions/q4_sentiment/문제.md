...
4. **불균형 처리**: train 데이터의 클래스 불균형을 해소하세요.

5. **TF-IDF 벡터화**: `sklearn`의 `TfidfVectorizer`를 사용하세요.

6. **모델 학습**: `LogisticRegression(random_state=42)`으로 학습하세요.
...
| 파일 | 설명 |
|------|------|
| `reviews.csv` | `text` 열(리뷰 텍스트)과 `label` 열(0 또는 1) |
