...
다음 요구사항을 모두 충족하는 Python 스크립트 `q5_solution.py`를 작성하십시오.
...
7. **B-3. 2층 신경망 Forward Pass (NumPy 직접 구현)** — ★새 역량:
   ...
   - 2층 신경망의 Forward Pass를 NumPy로 직접 구현하세요.
   - 적절한 활성화 함수와 출력층 함수를 사용하세요.
   - ⚠ 수치 안정성에 유의하세요.
...
10. **개선 실험**:
    - 소수 클래스 분류 성능을 개선하는 기법을 적용하여, 적용 전/후 **Macro F1**을 비교하세요.
